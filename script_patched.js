...
function localRuleReply(q) {
  ...
  const fohsh = ["کیر", "کص", "کونی", "کون", "باسن", "ممه", "boob", "boobi", "میمی", " جنده", "ass", "asshole", "جندگی",
    "niple", "جق ", "jerking", "gooner", "cum", "گشاد ", "کص مادر", "دهن لق ", "خارکصه", "گایید", "خارتو گاییدم ",
  ...
  if (s === "thlgo87") {
    return "این یک کلید مخفی هست که میتونی فقط با دیدین کد جاوا اسکیریپتم ببینی یا اینکه از من دریافتش کردی که کل این کد برای رحیمی ساخته شده و همین و بس "
  }
  if (s === "ghazalschool1234") {
    return " این یک کلید مخفی هست که میتونی فقط با دیدین کد جاوا اسکیریپتم ببینی یا اینکه از خودم گرفتیش که کل این کد برای بچه های غزال ساخته شده و همین و بس و اینکه این سایت هیچ توهینی به شما گل پسر های غزال نیست"
  }
  if (s.includes("سایت محیط زیست") || s.includes("سایت خوب") || s.includes("سایت انسان و محیط زیست") || s.includes("پروژه انسان و محیط زیست")) {
    return "من برات یه چیز فوق العاده برای پروژه انسان و محیط زیست درس شیش دارم برو توی این سایته   https://ghorbaizz.github.io/lesson6/"
  }
  return null;
}
